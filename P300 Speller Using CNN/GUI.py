...
window = Tk()
window.title('BCI Speller GUI')
window.geometry('725x300')
window.file_path = ' '
data_path = 'C:/Users/prady/Downloads/BCI_Virtual_Keyboard/P300 Speller Using CNN/Test Data/'
# add empty label in row 0 and column 0
l0 = Label(window, text='     ')
l0.grid(column=0, row=0)

lbl = Label(window, 
            text = 'Welcome to BCI Speller',
            font = ('Arial Bold', 14),                       
            )
...
def clicked():
    window.file_path = askopenfilename(initialdir = path.dirname(data_path),
                               ...
                               title = 'Select character to load') 
...
lbl2.grid(column = 5, row = 3,sticky = W )
...
def predict():
    ...
    if model == 0:
        ...
        lbl3.grid(column = 5, row = 4,sticky = W)

        lbl4 = Label(window, 
                    text = 'Actual: '+ w,
                    font = ('Arial ', 14))
            
        lbl4.grid(column = 5, row = 6,sticky = W )


btn = Button(window, text='Predict', command=predict)
btn.grid(column = 5, row = 8, sticky=W)
